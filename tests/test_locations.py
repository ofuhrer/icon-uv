...
def native():
    return PointLocation('town', 46.8, 9.8, 1000)
...
def test_adjusted_daily_uses_target_surface_and_rolling_support(tmp_path):
    ...
    screened=compute_daily(ds,locations,['2026-09-06'],t,terrain_screened=True)
    assert ambient.entries[1]['uvi'] > 4
    assert screened.entries[1]['uvi'] == 0
    assert screened.entries[0]['uvi'] == ambient.entries[0]['uvi']
...
def test_catalog_normalizes_points_and_isolated_metadata(tmp_path):
    ...
    assert c.catalog['entries'][0]['treatment']=='native'
    t=AnalyticTable();t.sha256='a'*64
